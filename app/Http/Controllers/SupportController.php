...
class SupportController extends Controller
{
    ...
    public function updateStatus(Request $request, Ticket $ticket)
    {
        ...
        if (!auth()->user()->hasRole('superadmin') && !auth()->user()->hasRole('manager')) {
            return redirect()->back()->with('error', 'Only superadmins and managers can update the status.');
        }
        // Prevent reopening a closed ticket
        if ($ticket->status == 'closed') {
            return redirect()->back()->with('error', 'Closed tickets cannot be reopened or changed.');
        }
        ...
        $ticket->save();
        $admin = $ticket->admin->user;
        $admin->notify(new TicketStatusUpdateNotification($ticket, $oldStatus, $newStatus, getAuthenticatedUser()));
        // Send notification
