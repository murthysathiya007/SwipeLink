...
class PaymentsController extends Controller
{
    ...
    public function searchInvoices(Request $request)
    {
        ...
        $perPage = 10;
        $invoices = $this->workspace->estimates_invoices('invoice');

        $invoices = $invoices->when($query, function ($queryBuilder) use ($query) {
        ...
        $results = $invoices->map(function ($invoice) {
            return ['id' => $invoice->id, 'text' => ucwords(get_label('invoice_id_prefix', 'INVC-') . $invoice->id)];
        });

        // Flag for more results
        $pagination = ['more' => $invoices->count() === $perPage];
